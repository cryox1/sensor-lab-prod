...
ESP8266 (DHT11, CCS811, …) → MQTT (Mosquitto) → Kafka (Redpanda) → TimescaleDB → FastAPI + Next.js.

A small but real reference of the current industry-standard IoT pipeline,
sized for a homelab.

## Architecture

```
ESP8266 ──MQTT──▶ Mosquitto ──▶ bridge ──Kafka──▶ Redpanda
                                                    │
                                                    ├──▶ tsdb-writer ──▶ TimescaleDB
                                                    │                       ▲
                                                    └──────────────────── api ┘
                                                                          │
                                                                          ▼
...
`bridge/` is the MQTT→Kafka shim, `tsdb-writer/` is the Kafka→Postgres
sink, `api/` is the FastAPI read + WebSocket server, `web/` is the Next.js
dashboard.
...
- `http://localhost:9580` — Redpanda Console (Kafka topic inspector)
- API at `http://localhost:8000`, MQTT on `1883`

Internal-only services (no host port): redpanda (Kafka), timescaledb (Postgres).
The bridge, writer, and api reach them on the compose network.
...
| redpanda-console | 9580      | Kafka topic inspector UI            |
| timescaledb      | —         | Postgres + Timescale, internal only |
| bridge           | —         | MQTT → Kafka                        |
| tsdb-writer      | —         | Kafka → TimescaleDB                 |
...
```bash
# Point the deploy at your server (or edit deploy/deploy.sh directly).
...
Pre-reqs: an SSH key for the server, `ssh "$SENSOR_LAB_HOST"` resolves, Docker installed
on the remote. New to this? **`DEPLOYMENT.md`** is a step-by-step setup walkthrough
(and is written so an AI agent can run the interview with you).

> ⚠️ **Data safety — read `AGENTS.md`.** The production TimescaleDB was wiped once by
> a deploy that overwrote the server's compose and `rsync --delete`'d the tree. The
> live data lives under `${SENSOR_DATA_DIR}` on the server (bind-mounted into the
> stateful containers). `deploy.sh`
> now **backs up the DB first (and aborts if the backup fails)**, syncs only source dirs
> with **no `--delete`**, and **never** overwrites the server's `docker-compose.yml` /
> `.env` or touches data dirs — it only rebuilds the app services, leaving
> `timescaledb` / `redpanda` / `mosquitto` running.
>
> - Back up manually any time: `./deploy/backup.sh` → `${SENSOR_DATA_DIR}/backups`
>   (on a server, wherever `SENSOR_DATA_DIR` points). Restore: `./deploy/restore.sh <file>`.
...
- `firmware/neo6m_c3_oled/` — NEO-6M GPS + onboard 0.42″ OLED, device `gps01`, static IP `10.0.0.66`. **ESP32-C3, not ESP8266** (board package `esp32`, board "ESP32C3 Dev Module"; enable *USB CDC On Boot*). Publishes `lat`/`lon` (+ `alt_m`/`sats`/`speed_kmh`) once it has a fix. Uses `secrets.h` for `WIFI_SSID`/`WIFI_PASS`/`MQTT_HOST`, same as the other sketches. Wiring: GPS `VCC→3V3` (or 5V if your breakout regulates), `GND→GND`, **`TX→GPIO20`** (required), `RX→GPIO21` (optional). OLED is onboard on I²C `SDA=GPIO5`/`SCL=GPIO6`, addr `0x3C`.
...
- Kafka: `sensors.telemetry` (key = device_id), DLQ: `sensors.telemetry.dlq`
...
{"device_id":"gps01","ts":1714000000,"lat":51.5074,"lon":-0.1278,"alt_m":35,"sats":9,"speed_kmh":0}
...
`timescaledb/init.sql`). GPS sensors are plotted on the **`/gps`** map view
(1h/7d/30d).
...
# 2. Kafka topic getting messages? — Redpanda Console at :9580,
...
**DLQ.** `sensors.telemetry.dlq` collects malformed payloads — check it in
Redpanda Console if a known-good device's data isn't landing.
...
**Fresh deploy but no schema.** `timescaledb/init.sql` only runs on first
init of an empty volume. On an existing volume the `tsdb-writer` re-runs
