// Headless deep-sleep DHT11 node (ESP8266 NodeMCU / ESP-12F, device indoor02).
//
// Battery-first build: wake -> read DHT11 -> publish one MQTT message -> deep
// sleep (~20 uA). No display on this board anyway.
//
// ** REQUIRES one wire: D0 (GPIO16) -> RST ** so the timer can wake the chip.
// 470 ohm in series is recommended so USB flashing still works; otherwise lift
// the D0<->RST jumper while uploading.
//
