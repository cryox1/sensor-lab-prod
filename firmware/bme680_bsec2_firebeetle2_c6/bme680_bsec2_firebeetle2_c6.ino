// Always-on BME680-only indoor air node on DFRobot FireBeetle 2 ESP32-C6 /
// DFR1075 (device air05): a single BME680 driven by Bosch's BSEC2 IAQ
// algorithm. USB-powered permanently -- NO battery, NO deep sleep.
//
// This is the ENS160-less sibling of bme680_ens160_aht21_firebeetle2_c6
// (air04): with BSEC2 the BME680 alone covers everything the ENS160+AHT21
// combo provides -- temp/humidity (heat-compensated), a CO2 estimate
// (co2_eq_ppm ~ eco2_ppm), a VOC estimate (bvoc_eq_ppm ~ tvoc_ppb, in ppm
// not ppb) and an air-quality index (static IAQ 0-500 instead of AQI 1-5) --
// plus pressure on top. Always-on is required: BSEC2 samples the BME680
// every 3 s (LP rate) to run its gas baseline / IAQ calibration; a sleep
// cycle would keep resetting it. The calibration state is persisted to NVS
// (Preferences namespace "bsec") whenever accuracy reaches 3 and every 6 h
// after, so a reboot resumes calibrated. iaq_acc: 0 = stabilizing,
// 1 = uncertain, 2 = calibrating, 3 = calibrated.
//
...
//
// WIRING -- BME680 (SEN0248) WITHOUT the Gravity cable, via its 6 SPI pads,
// in real SPI mode. I2C over those pads is IMPOSSIBLE on this board
// (schematic V1.0): the MOSI pad goes through an always-enabled
// unidirectional 74HC125 gate into SDI, so the sensor's ACKs/read data can
// never reach the pad -- the bidirectional SDA level shifter (SI2302) sits
// only on the Gravity connector's D pin. The pads ARE the intended SPI
// connector. Per BME680 datasheet 6.1 the first LOW edge on CSB latches the
// chip into SPI mode until power-on-reset -- exactly what we want here.
//   VCC->3V3  GND->GND
...
// Libraries: bsec2 (+ dep BME68x Sensor library), PubSubClient.
...
// Bosch examples -- tune by comparing a few hours of temp_c against a trusted
// nearby node (air03/air04) and adjust.
...
IPAddress STATIC_IP   (10, 0, 0, 74);   // free address on your LAN
...
#define DEVICE_ID      "air05"
#define MQTT_TOPIC     "sensors/lab/" DEVICE_ID "/telemetry"
#define MQTT_CLIENT_ID "esp-" DEVICE_ID
// BSEC payload (~230 B) + topic is tight against PubSubClient's 256 B default
...
// so it never reaches the DB. Watch with: mosquitto_sub -t 'sensors/lab/air05/debug'
...
void connectMqtt() {
  ...
  if (mqttClient.connect(MQTT_CLIENT_ID)) {
    ...
    snprintf(dbg, sizeof(dbg),
      "{\"boot_ms\":%lu,\"reset\":%d,\"bsec\":%d,\"iaq_acc\":%u,"
      "\"ip\":\"%s\",\"rssi\":%d,\"wifi_ms\":%lu,"
      "\"connects\":%lu,\"pub_ok\":%lu,\"pub_fail\":%lu}",
      millis(), (int)esp_reset_reason(), bsecOk ? 1 : 0, bsecIaqAcc,
...
void setup() {
  ...
  Serial.println();
  Serial.printf("Boot: %s on FireBeetle 2 ESP32-C6 (BME680/BSEC2 only, always-on)\n",
    DEVICE_ID);
  ...
  bsecOk = initBsec();
  if (!bsecOk) {
    Serial.println(F("BME680/BSEC2: FAILED"));
    Serial.println(F("  -> check SPI pads: SCLK->23 MOSI->22 MISO->21 CS->18, 3V3, GND"));
  }
...
void loop() {
  ...
  if (!bsecFresh) {
    Serial.println(F("no fresh BSEC data (check SPI pads 23/22/21/18)"));
    return;
  }
  ...
  if (!isnan(bsecTempC) && !isnan(bsecHum)) {
    n += snprintf(payload + n, sizeof(payload) - n,
      ",\"temp_c\":%.1f,\"humidity\":%.0f,\"heat_index_c\":%.1f",
      bsecTempC, bsecHum, heatIndexC(bsecTempC, bsecHum));
  }
