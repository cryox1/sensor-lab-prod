// Headless deep-sleep ENS160 + AHT21 air node (ESP8266 / HW-364A, device air02).
//
// Same as ens160_aht21_nodemcu_deepsleep but the sensors share the HW-364A OLED
// I2C bus (SDA=GPIO14/D5, SCL=GPIO12/D6). This build never drives the OLED.
//
...
//
// !! GAS-SENSOR CAVEAT: the ENS160 needs continuous warm-up for trustworthy
//    eCO2/TVOC/AQI; deep sleep + per-wake re-begin() makes those fields
//    unreliable (often warming-up/default). Temp + humidity (AHT21) are fine.
//    For real air-quality logging use the always-on ens160_aht21_oled build;
//    this variant is for battery + data-plumbing testing. !!
...
// ---- I2C (HW-364A OLED bus, shared with the sensors) ----
...
void setup() {
  ...
  if (isnan(t) && eco2 < 0) {
    Serial.println(F("All sensors failed (check I2C wiring D5/D6)"));
